using System; using System.Collections.Generic; using System.Linq; using System.Threading.Tasks; using ClinicBooking.Web.Data; using ClinicBooking.Web.Models; using Microsoft.AspNetCore.Mvc; using Microsoft.AspNetCore.Mvc.RazorPages; using Microsoft.AspNetCore.Mvc.Rendering; using Microsoft.EntityFrameworkCore;
...
namespace ClinicBooking.Web.Pages.Bookings
{
    public class AgendaModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostUpdateStatusAsync(int id, BookingStatus status, string rowVersionBase64, DateOnly date, int? doctorId, int? specializationId){
            ...
            if(!string.IsNullOrEmpty(rowVersionBase64)){ _db.Entry(entity).Property(b=> b.RowVersion).OriginalValue = Convert.FromBase64String(rowVersionBase64);} entity.Status = status; try{ await _db.SaveChangesAsync(); TempData["Message"]="Stato aggiornato.";} catch(DbUpdateConcurrencyException){ TempData["Message"]="Conflitto di concorrenza: la prenotazione è stata modificata da un altro utente."; }
